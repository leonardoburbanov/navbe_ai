...
| Tools | `src/navbe/mcp_app/tools.py` (`flow.*`) |
...
- `flow.create` / `flow.validate` / `flow.run` / `flow.status` / `flow.resume` / `flow.list_runs`
...
- [x] `flow.run` returns before multi-node execution completes
- [x] pydantic ValidationError normalized in `flow.validate`
